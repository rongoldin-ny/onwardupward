...
import { contactMember } from "@/app/actions/member-contact";

/**
 * "Contact <first name>" on a member's profile, for coaches.
 *
 * The outbound twin of CoachRequestForm, and deliberately not a <form> for the
 * same reason: it renders inside ProfilePage's outer autosave form, and
 * browsers reassociate a nested form's submit button with the outer one.
 */
export default function MemberContactForm({
  ...
  async function send() {
    setState("sending");
    setError(null);
    const result = await contactMember(profileId, message);
    if (result.error) {
  ...
  return (
    <div className="rounded-[20px] border border-border-1 bg-surface-2 p-5 text-left">
      <p className="text-[15px] font-bold text-cream">Message {firstName}</p>
      <p className="mt-1.5 text-[13px] leading-[1.5] text-secondary">
        Your coaching profile goes with it, so they know who&apos;s writing.
      </p>
      <textarea
        value={message}
        ...
        maxLength={2000}
        autoFocus
        placeholder="What you noticed in their profile, and how you could help."
